...
{
    [Info("SecurityCameras", "k1lly0u", "0.2.21", ResourceId = 0)]
    class SecurityCameras : RustPlugin
    {
        ...
        {
            CameraManager camera = RustNET.SpawnDroppedItem(cameraId, new Vector3(cameraData.position[0], cameraData.position[1], cameraData.position[2]), new Vector3(cameraData.baseRotation[0], cameraData.baseRotation[1], cameraData.baseRotation[2])).gameObject.AddComponent<CameraManager>();
        ...
        private void InitializeCamera(BasePlayer player, DroppedItem droppedItem)
        {
            ...
            CameraManager camera = RustNET.SpawnDroppedItem(cameraId, droppedItem.transform.position, droppedItem.transform.eulerAngles).gameObject.AddComponent<CameraManager>();
                ...
                case "add":
                    {
                        ...
                        if (permission.UserHasPermission(player.UserIDString, permPublic) && (args.Length > 2 && args[1].ToLower() == "public"))
                        {
                            int terminalId;
                            ...
                            placement.SetRequiredItem(cameraId, new Vector3(0, 90, 0), new Vector3(0, 0, -0.1f), true, true, null, InitializeCamera);
                        }
                        else
                        {
                            RustNET.ItemPlacement placement = player.gameObject.AddComponent<RustNET.ItemPlacement>();
                            placement.SetRequiredItem(cameraId, new Vector3(0, 90, 0), new Vector3(0, 0, -0.1f), true, false, InitializeCamera, null);
                        }
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                Camera = new ConfigData.CameraOptions
                {
                    ...
                    RustNETIcon = "http://www.chaoscode.io/oxide/Images/RustNET/cameraicon.png"
                },                
                Version = Version
            };
        }
